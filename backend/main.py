...
async def run_agent_and_stream(criteria: str, attributes: List[str], email: str):
    ...
    yield f"event: status\ndata: 🔍 Running Discovery Agent with criteria: {criteria}\n\n"
    await asyncio.sleep(1)
    ...
    if results:
        ...
        for idx, company in enumerate(results, start=1):
            yield f"event: status\ndata: ⚙️ Analyzing startup {idx} of {len(results)}: {company.get('Company Name', 'N/A')}\n\n"
            await asyncio.sleep(1)

    # -------------------------
    # Send Email
    # -------------------------
    yield "event: status\ndata: 📧 Generating final report and preparing email...\n\n"
    await asyncio.sleep(1)
    ...
    if status_code:
        yield f"event: status\ndata: 📬 Report successfully sent to {email}!\n\n"
    else:
        yield f"event: status\ndata: ❌ Failed to send email: {response_body}\n\n"
